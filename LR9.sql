...
--
select o.ORDER_NUM, p.DESCRIPTION, p.PRICE, p.QTY_ON_HAND
from PRODUCTS p join ORDERS o
on p.MFR_ID = o.MFR and p.PRODUCT_ID = o.PRODUCT
where description = 'Reducer';

create index idx_product_descr on products (description) include (price);
create index idx_orders_mfr_product on orders (mfr, product);


------------------------------ПРИМЕР-------------------------------------------------------------
select s.NAME, c.company, o.AMOUNT --0.0146241 --0.145838
from salesreps s join orders o
on s.EMPL_NUM = o.REP
join CUSTOMERS c
on c.CUST_NUM = o.CUST
where s.REP_OFFICE = 11 and c.company = 'J.P. Sinclair';

create index idx_s_rep_off on salesreps(rep_office) include(name);
create index idx_o_rep_cust on orders(rep, cust) include(amount);
create index idx_c_company on CUSTOMERS(company);
----------------------------------------------------------------------------------------------
...
--8.	Для запросов, разработанных в лабораторной работе № 3, покажите и проанализируйте планы запросов (стоимость запроса).
