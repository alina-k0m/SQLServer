...
--Лабораторная работа №6 – СУБД – 3 часа
go
...
create procedure add_customer @my_CUST_NUM int,
								@my_COMPANY varchar(20),
								@my_CUST_REP int,
								@my_CREDIT_LIMIT decimal (9,2)
as
begin 
	begin try
		insert into CUSTOMERS values (@my_CUST_NUM, @my_COMPANY, @my_CUST_REP, @my_CREDIT_LIMIT)
	end try
	begin catch
	declare @error_code int = @@ERROR;
		if @error_code = 2627 print 'Dublicate customer!';
		if @error_code = 947 print 'No such rep!';
		end catch
end;
go

exec add_customer 2223, 'IBA Corp', 986, 50000;
go

insert into CUSTOMERS values (2223, 'IBA Corp', 986, 50000);
go

--1.2.	Поиска клиента по части названия; если такого не нашлось — вывести сообщение.
create procedure find_customer @v_COMPANY_like varchar (20)
as
begin 
	select * from CUSTOMERS where COMPANY like concat(@v_COMPANY_like, '%');
	if @@ROWCOUNT = 0 print 'Ничего не нашлось'
end;
go

exec find_customer 'A';
go

--1.3.	Обновления данных клиента.
create procedure update_customer @my_CUST_NUM int,
								@my_COMPANY varchar(20),
								@my_CUST_REP int,
								@my_CREDIT_LIMIT decimal(9,2)
as
begin 
	update CUSTOMERS 
		set COMPANY = IsNULL(@my_COMPANY, COMPANY),
			CUST_NUM = isNull(@my_CUST_REP, CUST_REP),
			CREDIT_LIMIT = isNull(@my_CREDIT_LIMIT, CREDIT_LIMIT)
		where CUST_NUM = @my_CUST_NUM
end;
go

--1.4.	Удаления данных о клиенте; если у клиента есть заказы, и его нельзя удалить — вывести сообщение. 
--create procedure delete_customer @my_CUST_NUM int
--as
--begin
--	begin try
--		delete CUSTOMERS where CUST_NUM = @my_CUST_NUM;
--	end try
--	begin catch
--		if @@ERROR = 547 print 'Нельзя удалить клиента, у него есть заказы';
--	end catch
--end;
--go
...
as
begin
	declare @orders_num int = (select count(*) from ORDERS where CUST = @my_CUST_NUM);
	if (@orders_num = 0)
		delete CUSTOMERS where CUST_NUM = @my_CUST_NUM;
	else print 'Нельзя удалить клиента, у него есть заказы';
end;
go

exec delete_customer 2222;
exec delete_customer 2101;
go

--2.	Вызвать разработанные процедуры с различными параметрами для демонстрации.
--[2023_Komarovskaya_06] -> Programmability -> Stored Procedure -> Execute Stored Procedure... -> ... -> OK
--https://learn.microsoft.com/ru-ru/sql/relational-databases/stored-procedures/execute-a-stored-procedure?view=sql-server-ver16

--3.	Разработать пользовательские функции: 
--3.1.	Подсчитать количество заказов сотрудника в определенный период. 
--Если такого сотрудника нет — вернуть -1. Если сотрудник есть, а заказов нет — вернуть 0.
create function order_cnt (@v_rep int,
							@v_from date,
							@v_to date) returns int
as
begin
	return (select count(*) from ORDERS where REP = @v_rep
	and ORDER_DATE between @v_from and @v_to);
end;
go

select dbo.order_cnt (104,'2007-01-01', '2008-01-01');
go

select EMPL_NUM,	
		name, 
		dbo.order_cnt(EMPL_NUM, '2007-01-01', '2007-12-31') y_2007,
		dbo.order_cnt(EMPL_NUM, '2008-01-01', '2008-12-31') y_2008
from SALESREPS;
go

--3.2.	Подсчитать количество товаров различных производителей ценой выше указанной. 
alter function prod_by_mfr (@v_price money) returns table
as
	return 
		select MFR_ID, count(product_id) prod_cnt
		from PRODUCTS
		where PRICE > @v_price
		group by MFR_ID;

select * from dbo.prod_by_mfr(540);
go

--3.3.	Подсчитать количество заказанных товаров для определенного производителя.
create function cnt_ordered_prod_by_mfr (@v_mfr char(3)) returns int
as
begin
	declare @rc int;
	set @rc = (select sum(QTY) from ORDERS where MFR = @v_mfr)
	return @rc;
end;
go

select dbo.cnt_ordered_prod_by_mfr('ACI') cnt;

--4.	Вызвать разработанные функции различными способами с различными параметрами для демонстрации.
--https://learn.microsoft.com/ru-ru/sql/relational-databases/user-defined-functions/execute-user-defined-functions?view=sql-ser